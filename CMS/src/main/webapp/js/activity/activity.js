...
function select(){
    ...
    var arr = pkg.split("||");
    $("#price").text(arr[4]+"元");
    $("#hiddenProductId").val(arr[0]);
    $("#hiddenProductName").val(arr[1]);
    $("#hiddenChargingId").val(arr[2]);
    $("#hiddenChargingName").val(arr[3]);
    $("#hiddenPrice").val(arr[4]);
    $("#hiddenChargingDuration").val(arr[5]);
    $("#hiddenChargingPic").val(arr[6]);
}
...
function generateDialog(rowInfo) {
	...
	var pkg_list_value =activityDataList[productName];
	if(pkg_list_value){
		$('#pkg_list').val(pkg_list_value);
