				...
				columns : [ [ {
						...
						},
						{
							...
							formatter : function(value, row, index) {
								...
								var openTime = new Date(Date.parse(row.openTime));
					            var closeTime = new Date(Date.parse(row.closeTime));  
					            var sysTime = new Date(Date.parse(new Date()));
								if(sysTime < openTime) {
									return "待生效";
	...
	$('#openLive').click(function() {
		...
		$('#single_branchName').combobox({    
			...
			onLoadSuccess:function () {
				var singleBranchNameList = $('#single_branchName').combobox('getData');
				if(singleBranchNameList!=null && singleBranchNameList.length>0) {
					$("#single_branchName").combobox('select',singleBranchNameList[0].id);
				}
			},
		    editable : false
		});
		$('#single_hallName').combobox({
		    ...
		    textField:'hallName',
			onLoadSuccess:function () {
				var singleHallNameList = $('#single_hallName').combobox('getData');
				if(singleHallNameList!=null && singleHallNameList.length>0) {
					$("#single_hallName").combobox('select',singleHallNameList[0].id);
				}
			},
		    editable : false
		});
...
function submit_single_form() {
	var branchName = $('#single_branchName').combobox('getText');
	var branchId = $('#single_branchName').combobox('getValue');
	if(branchName == null || branchName == ''){
		$.messager.alert(titleInfo, '请选择分公司');
		return;
	}

	var hallName = $('#single_hallName').combobox('getText');
	var hallId = $('#single_hallName').combobox('getValue');
...
//		}
		$.each(row, function(index, item){
			...
			$('#renew_hallId').val(item.hallId);
			$('#renew_hallName').val(item.hallName);
...
//	}
	...
	var live = {
		...
		"branchName":$('#renew_branchName').val(),
		"hallName":$('#renew_hallName').val(),
		"hallId":$('#renew_hallId').val(),
		//"closeTime" : endTime,
...
function initRefundLive() {
	...
	$('#refundLive').click(function() {
        ...
        var sysTime = new Date(Date.parse(new Date()));  
		if(sysTime > closeTime){
			$.messager.alert(titleInfo, '已过期订单不能退订');
			return;
		}
		$.each(row, function(index, item){
			...
			$('#refund_hallName').val(item.hallName);
...
function refund_live(){
	...
	var mac = $('#refund_mac').val();
	var sn = $('#refund_sn').val();
	var live = {
...
function initDownLoad() {
	//	alert(1);
		...
		var liveorder = {};
		if (startTime != "") {
			liveorder.submitTime = startTime;
		}
		var stopTime = $('#stopTime').datebox("getValue");
		if (stopTime != "") {
			liveorder.openTime = stopTime;
		}
		liveorder.branchId =$('#currentUserBranchId').val();
		if(startTime==''||stopTime==''){
			$.messager.alert(titleInfo, '因数据量较大，导出excel，开通时间不能为空！');
			return;
		}
		
		$.ajax({
			url : "excel/liveExcel.json",
			async : true, // 
			type : "POST",
			dataType : "json",
			data : liveorder,
