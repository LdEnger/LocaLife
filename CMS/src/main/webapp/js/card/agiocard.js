		...
		buttons : [ {
			...
			handler : function() {
				...
				if(""!=$.trim(autoActiveTimeLength)){
					...
					$.messager.confirm('提示信息', "激活有效期<span style='font-size:20px;color:red;'>0</span>天，生成后不可修改", function(r) {
						if (r) {
							submit_excel();
						}
					});
...
function submit_single_form() {
	...
	if($('#s_editMac').val()==''||$('#s_editSn').val()==''){
		$.messager.alert(titleInfo, "麦币生成激活码必填mac,sn" );
		return false; 
	}
	...
	$.post("agiocard/add.json", card, function(data) {
		if (data.code == 1) {
...
function delCard(id) {
	$.messager.confirm('Warning', '您确定要删除?', function(r) {
		if (r) {
			...
			$.post("agiocard/update.json", card, function(data) {
				if (1 == data.code) {
					$.messager.show({
						title : titleInfo,
						msg : '删除成功！',
