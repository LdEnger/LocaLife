...
function initDownLoad() {
	//	alert(1);
		...
		$.ajax({
			url : "excel/bossExcel.json?start="+startTime+"&end="+stopTime+"&type="+qtype,
			async : true, // 
			type : "get",
			dataType : "json",
...
function getTotalTxt(){
	$.ajax({
		url : "boss/getTotalTxt.json",
		async : true, // 
		type : "get",
		dataType : "json",
		success : function(data) {
...
<body class="easyui-layout">
	<div data-options="region:'center',title:'boss订单详情列表'" class="regionCenter">
		...
		<div id="smsRe_toolbar">
			<form action="boss/showReport.html" id="queryForm">
				<label>开始时间：	<input id="start" name="start" value="${start}"/> <label>
				<label>结束时间：	<input id="end" name="end" value="${end}"/> <label>
				<label>类型：<select id ="q_type" name="q_type" >
					<option value="city" <c:if test="${q_type=='city'}"> selected</c:if>>按地市统计</option>
					<option value="boss" <c:if test="${q_type=='boss'}"> selected</c:if>>按分公司统计</option>
				</select></label>
				...
				<font color="red">先点查询，再导出页面结果会更快</font>
				<input type="button" id="excel_btn" class="btn btn-success" value="数据报告" onclick="getTotalTxt();">
			</form>
		</div>
		<table id="boss_excel">
