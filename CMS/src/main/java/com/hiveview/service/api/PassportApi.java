...
public class PassportApi {
	...
	public UserVo getUserInfo(String mac, String sn) {
		...
		synchronized (mac) {
			...
			try {
				...
				if (StringUtils.isEmpty(body) || "error".equals(body)) {
					return null;
				}
				...
				DATA.info("[getMacSn] body={},result={}", new Object[] { body, result});
				redisService.set(key, user, 1 * 180L, UserVo.class);// 缓存三分钟
				return user;
			} catch (Exception e) {
				e.printStackTrace();
				DATA.info("[getMacSn] error：macsn={}", new Object[] {e});
				return null;
			}
		}
	}
	...
	String getUserInfoHttp(String mac, String sn) {
		...
		if (hiveHttpResponse.statusCode != HttpStatus.SC_OK) {
			DATA.info("[getUserInfo] error:mac={},sn={},entityString={}", new Object[] { mac, sn, hiveHttpResponse.entityString });
			return "error";
		} else {
			String entityString = hiveHttpResponse.entityString;
			if(StringUtils.isEmpty(entityString)){
				DATA.info("[getUserInfo] error_empty:mac={},sn={},entityString={}", new Object[] { mac, sn, hiveHttpResponse.entityString });
			}
