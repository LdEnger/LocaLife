...
<script type="text/javascript" src="js/liveTask/live_task_list.js?v=2"></script>
		...
		<div id="task_dialog" data-options="closed:true,modal:true,title:'批量开通直播任务',iconCls:'icon-save'" style="padding: 5px; width: 400px; height: 400px;">
			<table>
				...
				<c:if test="${currentUser.roleId==1}">
					<tr>
						<td>分公司名称：</td>
						<td><input id = "batch_branchName" name="batch_branchName" class="easyui-combobox"  type="text" /></td>
					</tr>
				</c:if>
				<c:if test="${currentUser.roleId!=1}">
					<tr>
						...
						<td>分公司名称：</td>
						<td><input id = "batch_branchName" name="batch_branchName" style="color:#D3D3D3" type="text"  value="${currentUser.branchName}"  data-options="disabled:true" /></td>
					</tr>
					<tr>
						<td>营业厅名称：</td>
						<td><input id = "batch_hallName" name="batch_hallName" type="text"  style="color:#D3D3D3" value="${currentUser.hallName}" readonly="readonly"/></td>
					</tr>
				</c:if>
				<tr>
					<td>产品包名称：</td>
